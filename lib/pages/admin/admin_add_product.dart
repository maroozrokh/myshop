import 'dart:convert';
import 'dart:io';
import 'dart:typed_data';
import 'package:flutter/material.dart';
import 'package:myshop/pages/admin/admin_page.dart';
import 'package:myshop/core/constVariable.dart';
import 'package:myshop/core/show_model_sheet.dart';
import 'package:myshop/widgets/custom_widgets/custom_button_container.dart';
import 'package:myshop/widgets/custom_widgets/custom_textfield.dart';
import 'package:myshop/models/product_model/product_model.dart';
import 'package:myshop/controllers/product_controller/products_controller.dart';
import 'package:get/get.dart';
import 'package:myshop/controllers/tag_controller/tags_controller.dart';

class AdminAddProductView extends StatelessWidget {
  AdminAddProductView({Key? key, this.productModel}) : super(key: key);
  ProductModel? productModel;

  ProductsController productController = Get.find<ProductsController>();
  TagsController tagController = Get.find<TagsController>();

  @override
  Widget build(BuildContext context) {
    //  Get.put(ProductsController());

    // ImagePicker picker = ImagePicker();
    // XFile? image;
    // PickedFile? image ;
    //  final bytes = File(productController.image!.path).readAsBytesSync();
    //  String base64Image =  "data:image/png;base64,"+ base64Encode(bytes);

    // print("img_pan : $base64Image");

    var bytes;
    // String? base64Image;

    String BASE64_STRING =
        '';
    Image img = Image.memory(base64Decode(BASE64_STRING));
    Uint8List bytes1 = base64Decode(BASE64_STRING);

    return Scaffold(
        body: SafeArea(
      child: ListView(
        children: [
          Column(
            children: [
              Stack(
                children: [
                  InkWell(
                      child: Container(
                    width: 200,
                    height: 200,
                    child: ClipOval(
                      child: SizedBox.fromSize(
                        size: Size.fromRadius(48), // Image radius
                        child: InkWell(
                            onTap: () => {
                                  showModelSheet(
                                    context,
                                    func1: () => {
                                      productController.pickImageForProduct(
                                          productController)
                                    },
                                    func2: () => {
                                      productController.pickImageForProduct(
                                          productController)
                                    },
                                  ),
                                },
                            child: GetBuilder<ProductsController>(
                              builder: (ProductsController) {
                                return productController.image == null
                                    ? Image.asset(ConstaVariable.imageList![1])
                                    : Image.file(
                                        File(productController.image!.path));
                              },
                            )),
                      ),
                    ),
                  )),
                  Positioned(
                    top: 160,
                    left: 160,
                    child: InkWell(
                      child: Icon(
                        Icons.camera_alt_rounded,
                        size: 40,
                      ),
                    ),
                  )
                ],
              ),
              SizedBox(
                height: 30,
              ),
              CustomTextField(
                hintText: 'نام محصول',
                labelTxt: 'نام محصول',
                myController: productController.productNameController,
              ),
              CustomTextField(
                hintText: 'قیمت',
                labelTxt: 'قیمت',
                myController: productController.productpriceController,
              ),
              CustomTextField(
                hintText: 'تخفیف',
                labelTxt: 'تخفیف',
                myController: productController.productDiscountController,
              ),
              CustomTextField(
                hintText: 'تعداد کل',
                labelTxt: 'تعداد کل',
                myController: productController.productTotalCountController,
              ),
              CustomTextField(
                hintText: 'توضیحات',
                labelTxt: 'توضیحات',
                maxLines: 7,
                myController: productController.productDiscriptionController,
              ),
              CustomBtnContainer(
                colorBtn: ConstaVariable.kblue,
                height: 110,
                textBtn: 'انتخاب تگ',
                colorTxtBtn: ConstaVariable.kwhite,
                onTap: () => {
                  productController.showMultiSelectTag(context, tagController),
                },
              ),
              Obx(() => Wrap(
                  children: productController!.tagNameSelected
                      .map(
                        (tags) => InkWell(
                            onTap: () => {
                                  productController!.tagNameSelected
                                      .remove(tags),
                                },
                            child: Chip(
                              label: Text(tags),
                            )),
                      )
                      .toList())),
              SizedBox(
                height: 20,
              ),
              CustomBtnContainer(
                colorBtn: ConstaVariable.kblue,
                colorTxtBtn: ConstaVariable.kwhite,
                textBtn: productController.checkIfEditProductSelcted.value
                    ? 'ویرایش محصول'
                    : 'ذخیره محصول',
                height: 100,
                onTap: () => {
                  //   bytes = File(productController.image!.path).readAsBytesSync(),
                  //    productController.base64Image!.value =  "data:image/png;base64,"+ base64Encode(bytes),

                  // print("img_pan : ${productController.base64Image}"),
                  if (productController.checkIfEditProductSelcted.value ==  false)
                    {
                      productController.addProduct(
                          '',
                          productController.productNameController!.text,
                          productController.productDiscriptionController!.text,
                          int.parse(
                              productController.productpriceController!.text),
                          int.parse(productController
                              .productDiscountController!.text),
                          productController.tagNameSelected,
                          true,
                          int.parse(productController
                              .productTotalCountController!.text),
                          'category'),
                      productController.clearProductController(),
                      Get.to(AdminPageView()),
                    }
                  else
                    {
                      productController.setEditeDataToDB(),
                      productController.editProduct(),
                      productController.clearProductController(),
                      productController.checkIfEditProductSelcted.value = false,
                      Get.to(AdminPageView()),
                    }
                },
              ),
            ],
          )
        ],
      ),
    ));
  }
}
